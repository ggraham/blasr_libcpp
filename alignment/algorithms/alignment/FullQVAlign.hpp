...
double FullQVAlign(T_Query &query, T_Reference &target, Matrix<double> &alignProb)
{
    ...
    for (q = 1; q < numRows; q++) {
        for (t = 1; t < numCols; t++) {
            ...
            if (query.seq[q - 1] == target.seq[t - 1]) {
                matchedPulseProb =
                    (1 - query.GetSubstitutionQV(q - 1)) * (1 - target.GetSubstitutionQV(t - 1));
            } else {
                matchedPulseProb =
                    (query.GetSubstitutionQV(q - 1) / 3.0) * (1 - target.GetSubstitutionQV(t - 1)) +
                    ((1 - query.GetSubstitutionQV(q - 1))) *
                        (target.GetSubstitutionQV(t - 1) / 3.0);
            }
            ...
            if (target.GetDeletionTag(t - 1) != 'N') {
                ...
                if (q > 1) {
                    insertedPulseProb = (target.GetPreBaseDeletionQV(t - 1, query.seq[q - 2]) *
                                             target.GetDeletionQV(t - 1) +
                ...
                insertedPulseProb = (query.GetInsertionQV(q - 1) + target.GetDeletionQV(t - 1)) *
                                    exp(alignProb[q - 1][t]);
            }
            ...
            if (query.GetDeletionTag(q - 1) != 'N') {
                if (t > 1) {
                    deletedPulseProb = (query.GetPreBaseDeletionQV(q - 1, target.seq[t - 2]) *
                                            query.GetDeletionQV(q - 1) +
                                        target.GetInsertionQV(t - 1)) *
                                       exp(alignProb[q][t - 1]);
                } else {
                ...
                deletedPulseProb = (target.GetInsertionQV(t - 1) + query.GetDeletionQV(q - 1)) *
                                   exp(alignProb[q][t - 1]);
            }
