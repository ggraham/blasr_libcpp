...
int SDPHeaviestSubsequence(int queryLength, vector<T_Fragment> &fragmentSet, int indel, int match,
                           vector<int> &maxFragmentChain, AlignmentType alignType = Global)
{
    ...
    for (sweepX = 0; sweepX < queryLength; sweepX++) {
        ...
        while (fSweep < fragmentSet.size() and fragmentSet[fSweep].x == sweepX) {
            ...
            if (foundPrev and (alignType == Global or (alignType == Local and predFragScore < 0))) {
                ...
                fragmentSet[fSweep].cost =
                    fragmentSet[fSweep].GetLength() * match - fragmentSet[fSweep].weight;
