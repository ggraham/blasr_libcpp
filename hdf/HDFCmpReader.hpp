...
public:
    ...
    void ReadStructure(CmpFile &cmpFile)
    {
        ...
        for (refSeqIndex = 0; refSeqIndex < cmpFile.refGroup.path.size(); refSeqIndex++) {
            ...
            if (refAlignGroup == nullptr) {
                cout << "ERROR, unable to allocate memory for HDFCmpReader." << endl;
                exit(1);
            }
        ...
        for (alnGroupIndex = 0; alnGroupIndex < cmpFile.alnGroup.path.size(); alnGroupIndex++) {
            ...
            alnGroupPtr = refAlignGroups[refGroupArrayIndex]->InitializeExperimentGroup(
                readGroupName, includedFields);
    ...
    void Read(CmpFile &cmpFile)
    {
        ...
        for (alignmentIndex = 0; alignmentIndex < cmpFile.alnInfo.alignments.size();
            ...
            for (fieldIt = includedFields.begin(); fieldIt != fieldEnd; ++fieldIt) {
                ...
                HDFArray<float> *fieldArrayPtr = dynamic_cast<HDFArray<float> *>(
                ...
                cmpFile.alnInfo.alignments[alignmentIndex].StoreFloatField(*fieldIt, &fieldArray[0],
                                                                           alignmentLength);
            }
        }
    }
    ...
    void ReadAlignment(int alignmentIndex,
