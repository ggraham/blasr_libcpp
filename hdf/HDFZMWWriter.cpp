...
bool HDFZMWWriter::InPulseCalls(void) const { return inPulseCalls_; }
...
bool HDFZMWWriter::WriteOneZmw(const PacBio::BAM::BamRecord& read)
{
    ...
    _WriteHoleXY(static_cast<int16_t>(hn_ >> 16), static_cast<int16_t>(hn_ & 0x0000FFFF));
...
bool HDFZMWWriter::WriteFakeDataSets(void)
{
    ...
    return __WriteFake2DDataSet<float>(zmwGroup_, PacBio::GroupNames::baselinelevel, arrayLength_,
                                       4, 0) and
           __WriteFake2DDataSet<float>(zmwGroup_, PacBio::GroupNames::baselinesigma, arrayLength_,
                                       4, 0) and
           __WriteFake2DDataSet<float>(zmwGroup_, PacBio::GroupNames::signallevel, arrayLength_, 4,
                                       0) and
           __WriteFake2DDataSet<float>(zmwGroup_, PacBio::GroupNames::signalsigma, arrayLength_, 4,
                                       0);
}
...
bool HDFZMWWriter::_WriteBaseLineSigma(const PacBio::BAM::BamRecord& read)
{
    ...
    if (HasBaseLineSigma()) {
        if (read.Impl().HasTag("bs")) {
            ...
            if (data.size() != 4) {
                AddErrorMessage(
                    std::string("Tag BaseLineSigma must have 4 values per each record in read " +
                                read.FullName()));
            }
            float bls[4];
            bls[baseMap_['A']] = data[0];
            bls[baseMap_['C']] = data[1];
            bls[baseMap_['G']] = data[2];
            bls[baseMap_['T']] = data[3];
...
void HDFZMWWriter::_WriteAttributes(void)
{
    ...
    if (holeNumberArray_.IsInitialized() and holeNumberArray_.size() > 0) {
        AddAttribute<std::string>(holeNumberArray_, PacBio::AttributeNames::Common::description,
                                  PacBio::AttributeValues::ZMW::HoleNumber::description);
    }
...
*/
    ...
    if (holeXYArray_.IsInitialized() and holeXYArray_.GetNRows() > 0) {
        AddAttribute<std::string>(holeXYArray_, PacBio::AttributeNames::Common::description,
                                  PacBio::AttributeValues::ZMW::HoleXY::description);
    }

    if (HasBaseLineSigma() and baseLineSigmaArray_.GetNRows() > 0) {
        AddAttribute<std::string>(baseLineSigmaArray_, PacBio::AttributeNames::Common::description,
                                  PacBio::AttributeValues::ZMW::BaseLineSigma::description);
    }
}
#endif
